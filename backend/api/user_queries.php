...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $stmt = $conn->prepare("INSERT INTO user_queries 
    (fullName, email, contactNumber, pincode, city, query, date)
    VALUES (?, ?, ?, ?, ?, ?, NOW())");
...
if ($_SERVER['REQUEST_METHOD'] === 'PUT') {
  ...
  $stmt = $conn->prepare("UPDATE user_queries SET 
    fullName = ?, email = ?, contactNumber = ?, pincode = ?, city = ?, query = ?
    WHERE id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['id'])) {
  $id = intval($_GET['id']);
  $stmt = $conn->prepare("SELECT * FROM user_queries WHERE id = ?");
  $stmt->bind_param("i", $id);
  $stmt->execute();
