...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
  if (isset($_GET['id'])) {
    $id = intval($_GET['id']);
    $stmt = $conn->prepare("SELECT * FROM carousel_images WHERE id = ?");
    if (!$stmt) {
      http_response_code(500);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  $stmt = $conn->prepare("INSERT INTO carousel_images (image_url) VALUES (?)");
  if (!$stmt) {
    http_response_code(500);
    echo json_encode(["error" => $conn->error]);
    exit();
...
if ($_SERVER['REQUEST_METHOD'] === 'PUT') {
  ...
  if (!isset($data['id']) || !isset($data['image_url'])) {
    http_response_code(400);
    echo json_encode(["error" => "Missing id or image_url"]);
    exit();
  }

  $stmt = $conn->prepare("UPDATE carousel_images SET image_url = ? WHERE id = ?");
  if (!$stmt) {
    http_response_code(500);
    echo json_encode(["error" => $conn->error]);
    exit();
  ...
  $stmt->bind_param("si", $data['image_url'], $data['id']);
  if ($stmt->execute()) {
